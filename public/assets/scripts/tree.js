export default function define(runtime, observer) {
	...
	let linkwieght = d3.scaleLinear().range([13, 1]).domain([0, 11]);
    ...
    function fillStats(nodes) {
        ...
        var selected = d3
			...
			selected = d3.select("#commonNames").append("ul").classed("text-sm space-y-2 list-disc list-inside dark:text-gray-800", true);
			selected
            .selectAll("li")
			...
			selected = d3.select("#mostreached").append("ul").classed("text-sm space-y-2 dark:text-gray-800", true);
			selected
            .selectAll("li")
            .data(mostreach)
            .enter()
    ...
    main.variable(observer("chart")).define("chart", ["d3"], function* (d3) {
        ...
        svg = d3.create("svg").style("max-width", "100%").style("height", "auto").style("font", "bold 8px 'Cairo',sans-serif").style("margin", "5px");
